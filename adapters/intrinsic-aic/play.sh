...
# GPU: match upstream compose's `gpus: all` pattern. Eval mode uses
# this verbatim with no GPU-selection logic and it works fine. The
# fancy display-GPU detection we used to do here was added to fix a
# different bug (model not finding any GPU); it's not needed for
# rendering.
...
# X11 / render hints only when a window is actually being shown.
# Headless Gazebo (GUI=false RVIZ=false) doesn't need any of this;
# adding the env vars on a headless run can trigger EGL paths that
# slow down rendering on hybrid-GPU hosts.
if [[ "$GUI" == "true" || "$RVIZ" == "true" ]]; then
  EVAL_DOCKER_ARGS+=(
    -e "DISPLAY=${DISPLAY:-:0}"
...
# stable path (matches eval.sh wiring). Policy code writes PPMs here.
# Wipe before each run so old frames don't confuse debugging.
...
# Pre-wipe vision subdir so each Play run starts with a clean slate.
